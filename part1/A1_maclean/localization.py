...
class Particle(object):
  ...
  def __init__(self):
    ...
    # Initialize a particle randomly in the arena. Set the values of
  ...
  def is_valid(self):
    # Function that returns True if the current particle position is valid.

    # Check within walls
    if np.abs(self._pose[X]) > WALL_OFFSET or np.abs(self._pose[Y]) > WALL_OFFSET:
      return False

    # Check outside cylinder
    if np.linalg.norm(self._pose[[X, Y]] - CYLINDER_POSITION) < CYLINDER_RADIUS:
      return False
  ...
  def move(self, delta_pose):
    ...
    # Forward displacement u
    u = delta_pose[X] * np.random.normal(1, 0.1)

    # Rotational change w
    w = delta_pose[YAW] * np.random.normal(1, 0.1)
  ...
  def compute_weight(self, front, front_left, front_right, left, right):
    ...
    probs = np.exp(-1/2 * (((distances-sensor_distances) ** 2) / variance)) / (sigma * np.sqrt(2 * np.pi))
